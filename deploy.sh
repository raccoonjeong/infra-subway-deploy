...
function find_process() {
  echo_empty_line
  echo -e "${txtgrn}>> Find Processs Id ${txtrst}"
...
function start() {
  echo_empty_line
  echo -e "${txtgrn}>> start program ${txtrst}"
  nohup java -jar -Dspring.profiles.active=prod ./build/libs/subway-0.0.1-SNATSHOT.jar 1> application.log 2>&1 &
}
